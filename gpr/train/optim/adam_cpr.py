...
class AdamCPR(Optimizer):
    def __init__(self,
        ...
        defaults = dict(lr=lr, betas=betas, eps=eps, weight_decay=0.0,
                        regularize=False, kappa_update=kappa_update,
                        amsgrad=amsgrad,
                        maximize=maximize, foreach=foreach, capturable=capturable,
                        differentiable=differentiable)

        if isinstance(params, torch.nn.Module):
            params = group_parameters_for_cpr_optimizer(params, regularize_embedding=False)
        ...
        if regularize:
            ...
            if (kappa_init_method == 'inflection_point'):
                ...
                if  kappa == 1000:
                    ...
                    inflection_point_ema = ema_decay * inflection_point_ema + (1 - ema_decay) * current_l2m
                    ...
                    if step > iteration * 1:
                        ...
                        if step > iteration * 3 and prev_reg_gradient > current_reg_gradient:
                            kappa.mul_(0).add_(current_l2m)

                        # Update previous values for next iteration
                        prev_reg = inflection_point_ema
                    ...
                    lagmul.add_(param_specific_lagmul_rate * constraint_value).clip_(min=0.)
                    if add_lr: # TODO
                        param.addcmul_(param, lagmul * lr, value=-2)
                    else:
                        param.addcmul_(param, lagmul, value=-2)
...
def _multi_tensor_adam(params: List[Tensor],
    ...
    grouped_tensors = Optimizer._group_tensors_by_device_and_dtype(
        [params, grads, exp_avgs, exp_avg_sqs, max_exp_avg_sqs, lagmuls, kappas, kappa_updates, prev_regs, prev_reg_gradients, inflection_point_emas, state_steps])
    for ((
        device_params,
        device_grads,
        device_exp_avgs,
        ...
        device_inflection_point_emas = [torch.view_as_real(x) if torch.is_complex(x) else x for x in inflection_point_emas]
        ...
        if regularize:
            if kappa_init_method == 'inflection_point':
                ...
                square_sum_params, square_mean_params, ns = [], [], []
                for square_param in square_params:
                    square_sum_params.append(square_param.sum().unsqueeze(0))
                    square_mean_params.append(square_param.mean().unsqueeze(0))
                ...
                if any([device_kappa == 1000 for device_kappa in device_kappas]):
                    ...
                    for i in range(len(device_params)):
                        device_inflection_point_emas[i] = ema_decay * device_inflection_point_emas[i] + (1 - ema_decay) * square_mean_params[i]
                    if device_state_steps[0] % iteration == 0 and device_state_steps[0] > iteration * 1 :
                        ...
                        if device_state_steps[0] > iteration * 3:
                            for i in range(len(device_params)):
                                if device_prev_reg_gradients[i] > current_gradients[i] and device_kappas[i] == 1000:
                                    device_kappas[i].copy_(square_mean_params[i])
